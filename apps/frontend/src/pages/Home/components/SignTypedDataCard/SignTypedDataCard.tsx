...
export const SignTypedDataCard = () => {
    ...
    const onSignV1 = useCallback(async () => {
        if (!window.vechain) {
            return
        }
        const wallet = new ExtendedCertificateBasedWallet( // -> dappkit
            window.vechain?.newConnexSigner(
                getConfig("testnet").network.genesis.id,
            ), // -> veworld + estension
        )
    ...
    const onSignV2 = useCallback(async () => {
        if (window.vechain) {
                    ...
                    <VStack align="start" spacing={2}>
                        <HStack>
                            <Button
                                ...
                                size="sm"
                                onClick={onSignV2}>
                                Sign with connected wallet V2
                            </Button>
                            <Button
                                colorScheme="yellow"
                                size="sm"
                                onClick={onSignV1}>
                                Sign with connected wallet V1
                            </Button>

                            <Button
                                colorScheme="gray"
                                size="sm"
                                onClick={signWithFakeWallet}>
